...
function RegistroProblemas() {
  ...
  const [occurrenceId, setOccurrenceId] = useState(null);  // Defina occurrenceId aqui ou obtenha de alguma forma
  ...
  // Carregar imagens da ocorrência específica
  useEffect(() => {
    const storedImages = JSON.parse(localStorage.getItem("imagesByOccurrence")) || {};
    setImagens(storedImages[occurrenceId] || []);
  }, [occurrenceId]);

  const handleRemoveImage = (index) => {
    const storedImages = JSON.parse(localStorage.getItem("imagesByOccurrence")) || {};

    if (storedImages[occurrenceId]) {
      storedImages[occurrenceId] = storedImages[occurrenceId].filter((_, i) => i !== index);
      localStorage.setItem("imagesByOccurrence", JSON.stringify(storedImages));
      setImagens(storedImages[occurrenceId] || []);
    }
  };
  ...
  const handleAddImages = (event) => {
    ...
    if (files.length > 0) {
      ...
      Promise.all(promises).then((base64Images) => {
        const storedImages = JSON.parse(localStorage.getItem("imagesByOccurrence")) || {};
        storedImages[occurrenceId] = [...(storedImages[occurrenceId] || []), ...base64Images];
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
                <Box>
                  <Typography variant="body2">
                    <strong>Nome da Ocorrência:</strong> {o.descricao}
                  </Typography>
                  <Typography variant="body2">
                    <strong>Descrição da ocorrência:</strong> {o.observacoes}
                  </Typography>
                  <Typography variant="body2">
                    <strong>Tarefas Executadas pelo ADM:</strong> {o.tarefaEditada || 'Nenhuma tarefa registrada'}
                  </Typography>
